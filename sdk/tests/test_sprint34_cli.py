...
class TestTrustDegradation:
    ...
    def test_no_degradation_trusted(self) -> None:
        from backend.app.services.trust_degradation import (
            InteractionEvent,
            InteractionType,
            TaintLevel,
    ...
    def test_degradation_untrusted(self) -> None:
        from backend.app.services.trust_degradation import (
            InteractionEvent,
            InteractionType,
            TaintLevel,
        ...
        events = [
            ...
            InteractionEvent(
                step=2,
                interaction_type=InteractionType.CONTEXT_INJECTION,
                taint_level=TaintLevel.UNTRUSTED,
            ),
        ]
        timeline = sim.simulate(events)
        assert timeline.final_ceiling < 1.0
        assert len(timeline.degradation_events) == 1

    def test_degradation_injection_severe(self) -> None:
        from backend.app.services.trust_degradation import (
            InteractionEvent,
            InteractionType,
            TrustDegradationSimulator,
        ...
        events = [
            InteractionEvent(
                step=1,
                interaction_type=InteractionType.INJECTION_DETECTED,
            ),
        ]
        timeline = sim.simulate(events)
        assert timeline.final_ceiling == 0.5  # default injection_penalty = 0.50

    def test_degradation_irreversible(self) -> None:
        from backend.app.services.trust_degradation import (
            DegradationConfig,
            InteractionEvent,
            InteractionType,
        ...
        timeline = sim.simulate(events)
        # Trust should not recover after step 2
        assert timeline.snapshots[1].trust_ceiling <= timeline.snapshots[0].trust_ceiling

    def test_degradation_custom_config(self) -> None:
        from backend.app.services.trust_degradation import (
            DegradationConfig,
            InteractionEvent,
            InteractionType,
        ...
        timeline = sim.simulate(events)
        assert timeline.final_ceiling == pytest.approx(0.2)
    ...
    def test_timeline_serializable(self) -> None:
        ...
        events = [
            InteractionEvent(
                step=1,
                interaction_type=InteractionType.DELEGATION_HOP,
                agent_id="agent-a",
            ),
        ]
        timeline = sim.simulate(events)
